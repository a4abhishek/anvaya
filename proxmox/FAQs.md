...
**Q: Why did cloning create a second disk instead of resizing the first?**

* **The Symptom:** You specified `size = "100G"` in Terraform, but your VM booted with a small 8GB disk (from the template) and a separate, unmounted 100GB disk.
* **The Cause:** **Interface Mismatch.**
  * **Template:** Uses `scsi0` (Standard for Linux).
  * **Terraform:** You specified `virtio0` (or `sata0`).
  * **The Reality:** Proxmox treats `scsi` and `virtio` as different physical buses. It sees you asking for a *new* disk on a *new* bus, not a modification of the existing `scsi0` disk.
* **The Fix:** Match your Terraform config to the template. If the template uses `scsi0`, your Terraform resource must also target `scsi0` (or just `disk { storage = "local-lvm" }` to let the provider find the boot disk).
